...
import org.task_1.Product;
import org.task_1.Shop;
import org.task_1.Cart;
...
*/
    ...
    void lastProductsDisappearFromStore() {
        ...
        Cart cart = new Cart(shop);
        // Act
        int productCountInShop = shop.getProductsShop().get(0).getQuantity();
        for (int i = 0; i < productCountInShop + 1; i++) {
            cart.addProductToCartByID(1);
        }
        // Assert
        assertThat(cart.getTotalPrice()).isEqualTo(shop.getProductsShop().get(0).getPrice()
                * productCountInShop);
    }
    ...
    void deletedProductIsReturnedToShop() {
        ...
        cart.removeProductByID(1);
        // Assert
        assertThat(shop.getProductsShop().get(0).getQuantity()).isEqualTo(productCountInShop + 1);
    }
    ...
    @Disabled
    void advancedCheckCalculatingTotalPrice(){
        // Arrange (Подготовка)
        Shop shop = new Shop(getStoreItems());
        Cart cart = new Cart(shop);
        // Act (Выполнение)
